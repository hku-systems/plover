...
# Step 2. Start qemu (witness machine side)
...
sudo LD_LIBRARY_PATH=$VMFT_ROOT/rdma-paxos/target:$LD_LIBRARY_PATH x86_64-softmmu/qemu-system-x86_64 \
-boot c -m 2048 -smp 4 -qmp stdio -vnc :7 -name sentinel -enable-kvm -cpu qemu64,+kvmclock \
...
-incoming tcp:0:8888 -global kvm-apic.vapic=false \
